...
{
    /// <summary>
    /// select_produceline 的摘要描述
    /// </summary>
    public class select_produceline : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        string conStr = Enterprise.ConStr_produceline;
        string tbiHRS_ConStr = Enterprise.ConStr_produceline;
        ...
        private void CanAdvanceMode(HttpContext context)
        {
            ...
            if (Common.Get_IsLeaderOrChief(PA60002))
            {
        ...
        private DataTable DataTable4(string SQL3, HttpContext context)
        {
            ...
            //如果是組長核過就用PA600390去判斷  如果是一般的則用PA60039即可
            string o_approval = approval == "390" ? " PA600390 = '5' " : " PA60039 = '" + context.Request["approval"].ToString().Trim() + "' ";
            ...
            SQL3 = all_search(dept, user, leave, leavedate, toleavedate, approval, o_approval, context);
        ...
        public string all_search(string dept, string user, string leave, string leavedate, string toleavedate, string approval, string o_approval,HttpContext context)
        {
            user = user != "" ? user : context.Session["username"].ToString().Substring(1);
            SQL3 = "SELECT CONTEN, PA60002, CNname, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003, PA60038, PA60040, PA60039, PA60042 , PA600390 FROM TBI_HRS.dbo.PA123, tbi.dbo.Winton_mf2000 mf JOIN tbi.dbo.Users u ON zfbNO = mf.id WHERE SUBSTRING(Username, PATINDEX('%[^0]%', Username), LEN(Username)) = SUBSTRING(PA60002, PATINDEX('%[^0]%', PA60002), LEN(PA60002))  AND PA60043 IS NULL AND PA60004 NOT IN ('0') and PA60001 = '" + tbiHRS_ConStr + "' AND ('" + approval + "' = '-1' OR " + o_approval + ") ";
            //AND CONTEN IN (" + dept + ") AND PA60002 = '" + user + "' AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN '" + leavedate + "' AND '" + toleavedate + "' AND ('" + approval + "' = '-1' OR " + o_approval + ")

            //判斷單位有無選擇 有則加入SQL 沒則不用
            SQL3 = dept != "" ? SQL3 += " AND CONTEN IN (" + dept + ") " : SQL3;
            //判斷有無查詢不同使用者
            SQL3 += dept == "" ? " AND PA60002 = '" + user + "' " : "";

            leavedate = leavedate != "" ? leavedate : "1970-01-01";
            toleavedate = toleavedate != "" ? toleavedate : "2999-12-31";
            //判斷區間 分別: 1.起迄都選擇  2.只選擇迄  3.只選擇起  4.都不選擇
            if (leavedate != "1970-01-01" && toleavedate != "2999-12-31")
            {
                SQL3 += " AND '" + leavedate + "' between SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10)  AND SUBSTRING(CONVERT(varchar, PA60008, 120), 1, 10) ";
                SQL3 += " AND '" + toleavedate + "' between SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10)  AND SUBSTRING(CONVERT(varchar, PA60008, 120), 1, 10) ";
            }
            else if (leavedate == "1970-01-01" && toleavedate != "2999-12-31")
            {
                SQL3 += " AND '" + toleavedate + "' between SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10)  AND SUBSTRING(CONVERT(varchar, PA60008, 120), 1, 10) ";
            }
            else if (leavedate != "1970-01-01" && toleavedate == "2999-12-31")
            ...
            {
                SQL3 += " AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN DATEADD(year,DATEDIFF(year,0,GETDATE()),-365) /* 去年年初 */AND DATEADD(year,DATEDIFF(year,-1,getdate()),364) /* 明年年底 */ ";
            }

            //判斷有無選擇查詢假別
            SQL3 = leave != "" ? SQL3 += "AND PA60004 in (" + leave + ") " : SQL3;
        ...
        public void InitHr_MISPA51ANDTbiWPAAK_(HttpContext context)
        {
            ...
            string WPA51_sql = "select PA51002,PA51135 as PAAK002 from hrs_mis.dbo.wpa51 where PA51001='"+ Enterprise.ConStr_produceline + "' /*and PA51025 is null 加上後離職人員要查詢會跳Error*/";
        ...
        private DataTable PA60(HttpContext context)
        {
            ...
            if (approval == "-1" || approval == "0")
            {
                if (dept != "")//1.有選取部門
                {
                    if (user != "")//2.選取部門下查詢指定工號
                    {
                        ...
                        {
                            SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='"+Enterprise.ConStr_produceline+"' AND  PA60002 = '" + user + "' AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN '" + leavedate + "' AND '" + toleavedate + "' and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                            if (leave != "")//4.
                            {
                                SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='" + Enterprise.ConStr_produceline + "' AND PA60002 = '" + user + "' AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN '" + leavedate + "' AND '" + toleavedate + "' AND PA60004 IN (" + leave + ") and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                            }
                        }
                        else if (leavedate != "")//3.
                        {
                            SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='" + Enterprise.ConStr_produceline + "' AND PA60002 = '" + user + "' AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN '" + leavedate + "' AND '29991231' and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                            if (leave != "")//4.
                            {
                                SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='" + Enterprise.ConStr_produceline + "' AND PA60002 = '" + user + "' AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN '" + leavedate + "' AND '29991231' AND PA60004 IN (" + leave + ") and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                            }
                        }
                        else if (toleavedate != "")//3.
                        {
                            SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='" + Enterprise.ConStr_produceline + "' AND PA60002 = '" + user + "' AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN '19700101' AND '" + toleavedate + "' and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                            if (leave != "")//4.
                            {
                                SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='" + Enterprise.ConStr_produceline + "' AND AND PA60002 = '" + user + "' AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN '19700101' AND '" + toleavedate + "' AND PA60004 IN (" + leave + ") and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                            }
                        }
                        else if (leavedate == "" && toleavedate == "")//3.
                        {
                            SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='" + Enterprise.ConStr_produceline + "' AND PA60002 = '" + user + "' AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN DATEADD(mm, DATEDIFF(mm, 0, GETDATE()), 0) AND DATEADD(mm, 1, DATEADD(dd, 0, DATEADD(mm, DATEDIFF(mm, 0, GETDATE()), 0))) - 1 and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                            if (leave != "")//4.
                            {
                                SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='" + Enterprise.ConStr_produceline + "' AND PA60002 = '" + user + "' AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN DATEADD(mm, DATEDIFF(mm, 0, GETDATE()), 0) AND DATEADD(mm, 1, DATEADD(dd, 0, DATEADD(mm, DATEDIFF(mm, 0, GETDATE()), 0))) - 1 AND PA60004 IN (" + leave + ") and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                            }
                        }
                    }
                    else//2.選取部門下不選取工號
                    {
                        ...
                        for (int i = 0; i < doublecheck.Rows.Count; i++)
                        {
                            ...
                            if (leavedate != "" && toleavedate != "")//3.
                            {
                                SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='" + Enterprise.ConStr_produceline + "' AND PA60002 IN (" + num + ") AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN '" + leavedate + "' AND '" + toleavedate + "' and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                                if (leave != "")//4.
                                {
                                    SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='" + Enterprise.ConStr_produceline + "' AND PA60002 IN (" + num + ") AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN '" + leavedate + "' AND '" + toleavedate + "' AND PA60004 IN (" + leave + ") and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                                }
                ...
                else if (user == "")//1.查詢自己紀錄
                {
                    if (leavedate != "" && toleavedate != "")//2.
                    {
                        SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='" + Enterprise.ConStr_produceline + "' AND PA60002 = '" + PA60002 + "' AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN '" + leavedate + "' AND '" + toleavedate + "' and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                        if (leave != "")//3.
                        {
                            SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='" + Enterprise.ConStr_produceline + "' AND PA60002 = '" + PA60002 + "' AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN '" + leavedate + "' AND '" + toleavedate + "' AND PA60004 IN (" + leave + ") and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                        }
                    ...
                    else if (toleavedate != "")//2.
                    {
                        ...
                        if (leave != "")//3.
                        {
                            SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHER PA60001='" + Enterprise.ConStr_produceline + "' ANDE PA60002 = '" + PA60002 + "' AND SUBSTRING(CONVERT(varchar, PA60007, 120), 1, 10) BETWEEN '19700101' AND '" + toleavedate + "' AND PA60004 IN (" + leave + ") and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                        }
                    }
                    else if (leavedate == "" && toleavedate == "")//2.
                    {
                        SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='" + Enterprise.ConStr_produceline + "' AND PA60002 = '" + PA60002 + "' and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                        if (leave != "")//3.
                        {
                            SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE PA60001='" + Enterprise.ConStr_produceline + "' AND PA60002 = '" + PA60002 + "' AND PA60004 IN (" + leave + ") and PA60004 not in ('0') and PA60001 = '" + Enterprise.ConStr_produceline + "'";
                        }
                    }
                }
                DataSet ds = new DataSet();
            ...
            {
                SQL3 = "SELECT PA60002, PA60004, ISNULL(PA60007, '') PA60007, ISNULL(PA60008, '') PA60008, PA60011, PA60016, PA60998, PA60996, PA60003 FROM hrs_mis.dbo.WPA60 WHERE  '1' != '1' AND PA60001='" + Enterprise.ConStr_produceline + "' and PA60004 not in ('0')";
                DataSet ds = new DataSet();
                SqlConnection conn = new SqlConnection(hrs);
                SqlDataAdapter da = new SqlDataAdapter(SQL3, conn);
                da.Fill(ds);
        ...
        public void Search(HttpContext context)
        {
            ...
            for (int i = 0; i < pa60.Rows.Count; i++)
            {
                ...
                string sql2 = "SELECT TOP 1 PA25008 FROM hrs_mis.dbo.WPA25 WHERE PA25001='"+Enterprise.ConStr_produceline+"' AND PA25003 = '" + pa604 + "'";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                string sql2 = "SELECT TOP 1 PA25008 FROM hrs_mis.dbo.WPA25 WHERE PA25001='"+Enterprise.ConStr_produceline+"' AND PA25003 = '" + dt.Rows[i]["PA60004"].ToString() + "'";
                SqlConnection corr = new SqlConnection(hrs);//建立名為conn的連結物件
                corr.Open();
                SqlCommand tdiii = new SqlCommand(sql2, corr);
                DataTable doublecheck = new DataTable();
                ...
                row["PA600390"] = dt.Rows[i]["PA600390"].ToString();
                row["TYP"] = "TBIHRS";
