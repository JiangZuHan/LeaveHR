<%@ Page Title="" Language="C#" MasterPageFile="~/請假主版.Master" AutoEventWireup="true" CodeBehind="check1_produceline.aspx.cs" Inherits="全傳請假系統.check1_produeline" %>
    ...
    <div id="ontopDiv" class="container">
        ...
        <ul class="nav nav-tabs">
            ...
            <%if (Session["check1"] != null)
                ...
                {
                    Response.Write("<li role='presentation' id='check2' style='display: none' class='active' ><a href='" + Session["check2"] + "'>現場待審核</a></li>");
                }
            %>
        </ul>
    </div>
    ...
    <script> 
        ...
        $('#show').click(function () {
            ...
            if (check){
                ...
                for(var i = 0; i < size;i++){
                    ...
                    if(a == 'hrsum'){
                        ...
                        $.ajax({
                            type: "POST",
                            async:false,//啟用同步請求
                            url: 'Tools/leaveuser_produceline.ashx?user=' + Duser + '&water=' + Dwater + '&hr=' + Dhr + '&pass=yes&leave=' + encodeURI(Dleave) + '&death=' + Ddeath + '&start=' + encodeURI(Dstart) + '&message=' + a + '&check=' + check, //(檔案名/方法名稱)
                ...
                setTimeout("location.href = '<%Response.Write(Session["check2"]);%>';", 500);
            }
        });
        //-----------------------------------------------------------------------------------------
        $('#nopass').click(function () {
            ...
            if(nopasstext == ""){
                ...
                $.each($('input[name="selectItemName"]:checked'), function() {
                    ...
                    $.ajax({
                        type: "POST",
                        url: 'Tools/leaveuser_produceline.ashx?user=' + user + '&id=' + id +'&from='+ from + '&to=' + to +'&cause=' + cause +'&name=' + name + '&water=' + water + '&hr=' + hr + '&pass=no&nopasstext=' + encodeURI(nopasstext), //(檔案名/方法名稱)
                        contentType: 'application/json; charset=utf-8',
        ...
        function Deathcheck(Duser,Ddeath,hrsum,Dwater,Dstart){
            ...
            $.ajax({
                type:'post',
                async: false,//啟用同步請求
                url:'Tools/process_produceline.ashx?mode=DeathDateType&user='+Duser+'&hrsum='+Dhrsum+'&death='+Ddeath+'&Dwater='+Dwater+'&Dstart='+encodeURI(Dstart),
        ...
        function NoDeathYesPass(user,water,hr,leave,check){
            ...
            $.ajax({
                type: "POST",
                async:false,
                url: 'Tools/leaveuser_produceline.ashx?user=' + user + '&water=' + water + '&hr=' + hr + '&pass=yes&leave='+encodeURI(leave) + '&check=' + check, //(檔案名/方法名稱)
                contentType: 'application/json; charset=utf-8',
            }).then(function (response) {
                data = response;
            });    
        ...
        if( <%=  Convert.ToInt32(Session["CanCheck"]) %>==true)
            ...
            $.ajax({
                type:'POST',
                url:'Tools/leaveuser_produceline.ashx?pass=dptall',
                dataType:'json',
                contentType:'application/json; charset=utf-8',
                success:(function(dptall){ 
                    $.ajax({
                        type:'POST',
                        url:'Tools/leaveuser_produceline.ashx?pass=leaveall',
                        dataType: 'json',
                        contentType:'application/json; charset=utf-8',
                        success:(function(leave){
                            $.ajax({
                                type:'POST',
                                url:'Tools/leaveuser_produceline.ashx?pass=week',
                                dataType:'json',
                                contentType:'application/json; charset=utf-8',
                                    ...
                                    function test1($table, cells, rows) {
                                        ...
                                        function Approval(week2) {
                                            $('#ApprovalTable').bootstrapTable('refresh', {
                                                url: '/Tools/select_produceline.ashx?mode=Approval&week2=' + week2 + '&a=' + Math.random(),
                                                type: 'POST',
                                                contentType: 'application/json; charset=utf-8',
                                                dataType: 'json',
                                            });
            ...
            $.ajax({
                type: "POST",
                url: 'Tools/leaveuser_produceline.ashx?pass=bossone', //(檔案名/方法名稱)
                dataType: 'json',
                contentType: 'application/json; charset=utf-8',
            ...
            $.ajax({
                type:'POST',
                url:'Tools/leaveuser_produceline.ashx?pass=bos',
                dataType:'json',
                    ...
                    $('#table').bootstrapTable({data: data2,
                        ...
                        columns:[
                            ...
                            {field:'PA60040',title:'附件',
                               ...
                               {field:'PA60044',title:'喪假時數',class:'hiddencol'},
                                                              {field:'PA60048',title:'歸屬',class:'hiddencol'}
                        ]
                    });
                    if (data2.length == 1) {
